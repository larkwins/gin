...
| # | Issue | 标题 | 描述 | 涉及文件 | 修复难度 | 能否修复 |
|---|-------|------|------|----------|----------|----------|
| 1 | [#4572](https://github.com/gin-gonic/gin/issues/4572) | **非标准 X-Forwarded-For 头不被支持** | `c.ClientIP()` 无法识别带方括号的 IPv6（`[::1]`）、带端口的 IP（`1.2.3.4:8080`）等非标准格式，会错误回退到 `127.0.0.1` | `context.go` 中的 `ClientIP()` / IP 解析相关函数 | ⭐ 简单 | ✅ 能修复。只需在 IP 解析逻辑中增加 `net.SplitHostPort()` 和去除方括号的处理即可 |
